...
5. **ngrok** (public HTTPS URL for Slack → your laptop):
   ...
   ```
   Add **`NGROK_AUTHTOKEN`** (and optional **`NGROK_PORT`**, default `8000`) to your **`.env`** — copy from [ngrok dashboard → Your Authtoken](https://dashboard.ngrok.com/get-started/your-authtoken). Then start the tunnel from the project root:
   ```bash
   bash scripts/ngrok-start.sh
   ```
   (Equivalent to `ngrok http $NGROK_PORT --authtoken $NGROK_AUTHTOKEN`.) You can still use `ngrok config add-authtoken …` if you prefer; the script keeps the token next to your other env vars.
...
4. **Activate venv and run** (always **one** worker):
   ...
   ```
5. In another Terminal tab: **`bash scripts/ngrok-start.sh`** (reads **`NGROK_AUTHTOKEN`** from `.env`), then paste the HTTPS URL into Slack **Event Subscriptions** and **Interactivity** (append `/slack/events` and `/slack/interactivity`).
...
│   └── ngrok-start.sh     # ngrok using NGROK_AUTHTOKEN from .env
└── README.md
```
